...
const KanbanBoard = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        usersArray.forEach((user) => {
          ...
          if (user.status === "FrequentVisitor")
            columnData.frequentVisitor.push(user);
        });
  ...
  const DraggableItem = ({ user }) => {
    ...
    return (
      <div
        ...
        {...attributes}
        {...listeners}
        className="user-card bg-white p-4 rounded mb-4 shadow-md w-full hover:shadow-lg transition-shadow duration-300 ease-in-out"
      >
        <h3 className="text-md font-semibold mb-1">{user.name}</h3>
  ...
  const DroppableColumn = ({ columnId, children }) => {
    ...
    return (
      <div
        ref={setNodeRef}
        className="kanban-column bg-gray-100 p-5 rounded-lg shadow-md flex flex-col w-full md:w-1/4"
      >
  ...
  return (
    <DndContext onDragEnd={handleDragEnd}>
      <div className="kanban-board flex flex-col md:flex-row gap-6 p-4 overflow-x-auto">
        {loading ? (
      ...
      <DragOverlay>
        {activeId ? (
          <div className="user-card bg-white p-4 rounded shadow-md">
